  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-center gap-4 mb-6">
        <div className="h-[1px] w-12 sm:w-20 bg-gradient-to-r from-transparent to-[#FF4B12]/40" />
        <div className="w-10 h-10 rounded-full bg-[#FFE0C7] border border-[#FFC79E] flex items-center justify-center text-[#FF4B12] riso-shadow-sm">
          <Quote className={`w-5 h-5 ${isRtl ? 'scale-x-[-1]' : ''}`} />
        </div>
        <div className="h-[1px] w-12 sm:w-20 bg-gradient-to-l from-transparent to-[#FF4B12]/40" />
      </div>

      {/* Main Large Italic Fraunces Quote Body */}
      <blockquote className="text-center">
        <p className="font-heading italic text-2xl sm:text-3xl md:text-[32px] text-[#141B33] leading-[1.38] sm:leading-[1.42] tracking-tight max-w-3xl mx-auto font-normal">
          {quoteText}
        </p>
      </blockquote>
      ...
      {(authorText || roleText) && (
        <figcaption className="mt-6 text-center">
          {authorText && (
            <cite className="font-sans font-bold text-xs sm:text-sm uppercase tracking-[0.18em] text-[#FF4B12] not-italic block">
              — {authorText}
            </cite>
          )}
          {roleText && (
            <span className="text-xs text-[#7A6842] font-medium tracking-normal mt-1 block">
              {roleText}
            </span>
          )}
        </figcaption>
      ...
      <div className="mt-8 flex justify-center">
        <div className="h-[1.5px] w-16 bg-[#FF4B12]/30 rounded-full" />
      </div>
    </figure>
  );
};
